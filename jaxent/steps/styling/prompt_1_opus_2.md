...
3. **Forward model**
   - Icon: small rounded box (corner radius 6 px) with thin teal border (`#4CA6A8`), light teal fill (`#E0F2F1`), labeled "Forward model" (9 pt bold sans-serif) with subtext "BV (\beta_h, \beta_c)" in 8 pt serif italic for the variables, sans-serif for "BV".
   - Beneath: "(Best–Vendruscolo)" in 7 pt sans-serif, medium grey.

#### (a.3) Optional Input (bottom subsection connected to middle)
A **dashed-outline** rounded rectangle (corner radius 8 px, dash pattern 6-4, border `#8E99A4`, very faint cream fill `#FBF9F4`) labeled "Optional: orthogonal data for cross-validation" (9 pt regular sans-serif, medium grey). A dashed arrow (1.5 pt `#8E99A4`) exits rightward, feeding toward 'Outputs' in panel (c) - across the bottom of panel (b) (ensure the process steps do not overlap with this.).
...
- Small annotation right of the text (light teal fill): Matrix/heatmap showing "Residues x H-bonds, contacts" (7 pt sans-serif).
...
Symbols for error, work, constraints are above a funnel pointed into a gear/cog icon (dark blue)  
...
1. **Weight distribution** — tiny bar chart icon in teal tones, labeled *ω* (serif italic) above and "weight distribution" (7 pt sans-serif, grey) below. A 'reference distribtuion' is drawn behind the bars as a continuous distribution. This is what the arrow from (a) 'Orthogonal data for cross-validation' points to.
2. **Fitted parameters** — two tiny slider icons in coral tones, labeled \beta_h, \beta_c / *ω* (serif italic) above and "fitted parameters" (7 pt sans-serif, grey) below.
