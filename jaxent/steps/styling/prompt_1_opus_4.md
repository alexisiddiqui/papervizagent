...
## Caption
**Fig. X | jaxENT workflow for integrating biophysical experiments with structural ensembles via maximum-entropy reweighting.** (a) Candidate structural hypotheses (e.g., MD trajectories or AlphaFold2-derived ensembles) are linked to target experimental observables (e.g., HDX-MS uptake curves or NMR protection factors) through a forward model (here Best–Vendruscolo, parameterised by β_h, β_c, and ω), bridging the timescale gap between simulation (fs–ns) and experiment (ms–days). Orthogonal data may optionally be reserved for cross-validation. (b) The JAX-based fitting engine computes per-frame forward-model features (H-bonds, contacts), maps structures to protection factors and predicted uptake curves (with residue-to-peptide aggregation and back-exchange correction), then defines a composite objective combining data-fidelity (target reconstruction error), a maximum-entropy regulariser (Work), and optional physics-based priors or covariance constraints. Gradient-based optimisation (Adam, with autodiff and JIT compilation) simultaneously fits ensemble weights (ω) and forward-model parameters (β). (c) Built-in validation via data splits, replicates, and robust success metrics guards against overfitting and quantifies uncertainty. Candidate ensembles are scored using held-out error and information-theoretic work metrics (Work_KLD, Work_Scale), enabling quantitative hypothesis testing and selection of the best-supported structural model. Interpretable outputs include reweighted ensemble weights, fitted model parameters, and residue-level work-done maps for structural insight.
...
**Layout structure:** Three main left-to-right columns (panels) with bold panel labels **(a)**, **(b)**, **(c)** in the upper-left of each section. Panel labels: 14 pt bold sans-serif, dark navy (`#1A1A2E`). There is a line that crosses from (a.2) Optional Input to (c.3) Interpretable Outputs (reference distribution)
...
1. **Weight distribution** — tiny bar chart icon in teal tones, labeled *ω* (serif italic) above and "weight distribution" (7 pt sans-serif, grey) below. A 'orthogonal/reference distribtuion' is drawn behind the bars as a continuous but slightly shifted distribution. This is what the arrow from (a) 'Orthogonal data for cross-validation' points to.
