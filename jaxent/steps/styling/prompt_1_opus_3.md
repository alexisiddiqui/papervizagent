...
**Layout structure:** Three main left-to-right columns (panels) with bold panel labels **(a)**, **(b)**, **(c)** in the upper-left of each section. Panel labels: 14 pt bold sans-serif, dark navy (`#1A1A2E`). There is a line that crosses from (a.3) Optional Input to (c.3) Interpretable Outputs (reference distribution)
