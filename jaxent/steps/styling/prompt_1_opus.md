...
**Layout structure:** Three main left-to-right columns (panels) with bold panel labels **(a)**, **(b)**, **(c)** in the upper-left of each section. Panel labels: 14 pt bold sans-serif, dark navy (`#1A1A2E`).
...
| **Active / trainable elements** | Warm Coral | `#E8735A` | Optimization block border, ω/β icons, "Optimize" header accent |
...
A horizontal timescale bar/gauge rendered as a smooth **horizontal gradient bar** transitioning from teal (`#4CA6A8`, left) to warm coral (`#E8735A`, right):
- Left label: "Molecular dynamics (fs–ns)" — 9 pt sans-serif, dark navy.
- Right label: "HDX-MS experiments (ms–hours/days)" — 9 pt sans-serif, dark navy.
- Above the bar, centered: "*timescale gap*" in 9 pt italic sans-serif, medium grey.
- Below the bar, centered: "Integration acts as a statistical connector" — 8 pt italic sans-serif, medium grey.
- A thin line beneath the bar with labels "Input → Target" — 8 pt sans-serif, medium grey.

#### (a.2) Required Inputs Block (middle)
A large rounded rectangle (corner radius 8 px, white fill, thin charcoal border) labeled **"Required inputs"** (12 pt bold sans-serif, dark navy) containing three icon+label groups arranged in a row:
...
   - Below: "HDX-MS / NMR / SAXS" (8 pt regular, medium grey).

3. **Forward model**
   - Icon: small rounded box (corner radius 6 px) with thin teal border (`#4CA6A8`), light teal fill (`#E0F2F1`), labeled "Forward model" (9 pt bold sans-serif) with subtext "BV (*ε*_h, *ε*_c)" in 8 pt serif italic for the variables, sans-serif for "BV".
   - Beneath: "(Best–Vendruscolo)" in 7 pt sans-serif, medium grey.

#### (a.3) Optional Input (bottom callout)
A **dashed-outline** rounded rectangle (corner radius 8 px, dash pattern 6-4, border `#8E99A4`, very faint cream fill `#FBF9F4`) labeled "Optional: orthogonal data for cross-validation" (9 pt regular sans-serif, medium grey). A dashed arrow (1.5 pt `#8E99A4`) exits rightward, feeding toward panel (c).
...
**Panel subtitle** (below panel header): "compute → predict → objective → optimize" in 9 pt regular sans-serif, medium grey `#5A6270`.

#### (b.1) Feature Computation and Prediction Stack
Large rounded rectangle (white fill, charcoal border) titled **"jaxENT (JAX-based fitting engine)"** (12 pt bold sans-serif, dark navy) with a subheading "autodiff + JIT + modular losses" (9 pt regular sans-serif, medium grey).
...
- Small annotation to the right in a tiny rounded callout (light teal fill): "H-bonds, contacts" (7 pt sans-serif).
...
- **Header:** "Objective" (12 pt bold sans-serif, dark navy), with a thin gold underline beneath.
...
  - "MaxEnt Work" with a subtle gold background highlight pill behind it (very light gold `#FDF4E3`, rounded, inline) and a tiny annotation below: "MaxEnt regularizer" in 7 pt italic serif, medium grey.
  - "Data-fidelity" and "Priors/Constraints" in sans-serif regular.
- **Toggle/branch row** beneath (two side-by-side small boxes):
  - Left: "Error + Work" with label "(baseline)" in 7 pt grey below.
  - Right: "Error + Constraints + Work" with label "(covariance/structure-aware)" in 7 pt grey below.
...
A rounded rectangle with **warm coral accent**: thin coral border (`#E8735A`, 2 pt) or a coral top-stripe. White fill. Corner radius 8 px.

- **Header:** "Optimize" (12 pt bold sans-serif, dark navy).
...
  - "gradient-based (Adam)" — pill with light teal fill, teal border/text.
  - "autodiff" — same pill style.
  - "JIT" — same pill style.
...
**Panel subtitle:** "communicating robustness" in 9 pt regular sans-serif, medium grey.
...
- "Split types / held-out sets" — white fill, thin charcoal border, 9 pt sans-serif.
- "Replicates" — white fill, thin charcoal border, 9 pt sans-serif.
- "Cross-validation (optional orthogonal data)" — white fill, **dashed border** (`#8E99A4`) to visually match the optional input from panel (a), 9 pt sans-serif.
...
- **Left side:** Three mini protein-stack icons (flat 2D overlaid ribbon illustrations, each tinted a different pastel — teal, coral, lavender) labeled H₁, H₂, H₃ beneath in 8 pt sans-serif. Label below group: "Candidate ensembles" (8 pt regular, medium grey).
...
    - Work_KLD, Work_Scale, Work_Density (subscripts in serif italic)
    - Fit error (held-out)
...
1. **Weight distribution** — tiny bar chart icon in teal tones, labeled *ω* (serif italic) above and "weight distribution" (7 pt sans-serif, grey) below.
2. **Fitted parameters** — two tiny slider icons in coral tones, labeled *ε*_h, *ε*_c / *β* (serif italic) above and "fitted parameters" (7 pt sans-serif, grey) below.
3. **Structural mapping** — small protein cartoon colored by a teal-to-coral diverging colormap (implying predicted protection / uptake difference), labeled "predicted protection / uptake difference" (7 pt sans-serif, grey) below.
...
| Teal/Coral/Gold functional palette | Teal = process (cool), Coral = trainable/optimized (warm), Gold = objective/highlight — status-based coloring |
